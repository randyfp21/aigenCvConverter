...
export const AddCompanyModal: React.FC<AddCompanyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-2xl">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto backdrop-blur-3xl text-slate-900 dark:text-slate-100">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center space-x-2.5">
            <div className="w-9 h-9 rounded-2xl bg-blue-600/20 text-blue-600 dark:text-blue-400 border border-blue-500/30 flex items-center justify-center font-bold">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Kustomisasi logo, footer (alamat, website, telp), & warna separator</p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-xl text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="bg-slate-50 dark:bg-slate-950/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-1 flex items-center justify-center overflow-hidden shadow-sm">
                {logoUrl ? (
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl text-[11px] font-bold text-blue-600 dark:text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 flex items-center gap-1.5 shadow-sm"
            >
          ...
          <div>
            <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Nama Perusahaan / PT *</label>
            <input
              type="text"
              required
              value={companyName}
              ...
              className="w-full px-3.5 py-2.5 rounded-2xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 shadow-sm"
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Kode PT (Short Code)</label>
              <input
                type="text"
                value={code}
                onChange={(e) => setCode(e.target.value)}
                placeholder="AIGEN"
                className="w-full px-3.5 py-2.5 rounded-2xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 uppercase shadow-sm font-mono"
              />
            </div>
            <div>
          ...
          {/* Color Parameterizer */}
          <div className="bg-slate-50 dark:bg-slate-950/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 font-semibold">Warna Utama</label>
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <span className="text-[10px] text-slate-700 dark:text-slate-300 font-mono font-bold">{primaryColor}</span>
                </div>
              </div>

              <div>
                <label className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 font-semibold">Warna Separator</label>
                <div className="flex items-center space-x-2">
                  <input
                    type="color"
                    ...
                    className="w-8 h-8 rounded-lg border border-slate-300 dark:border-slate-700 bg-transparent cursor-pointer"
                  />
                  <span className="text-[10px] text-slate-700 dark:text-slate-300 font-mono font-bold">{separatorColor}</span>
                </div>
              </div>

              <div>
                <label className="block text-[10px] text-slate-500 dark:text-slate-400 mb-1 font-semibold">Warna Akses</label>
                <div className="flex items-center space-x-2">
                  <input
                    type="color"
                    ...
                    className="w-8 h-8 rounded-lg border border-slate-300 dark:border-slate-700 bg-transparent cursor-pointer"
                  />
                  <span className="text-[10px] text-slate-700 dark:text-slate-300 font-mono font-bold">{secondaryColor}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-slate-200 dark:border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-2xl font-bold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
            >
              Batal
            </button>
            <button
              type="submit"
              className="px-7 py-2.5 rounded-2xl font-black text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-lg shadow-blue-500/20 flex items-center space-x-1.5 transition-all transform active:scale-95"
            >
