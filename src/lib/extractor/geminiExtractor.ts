...
export async function uploadBufferToGemini(
  ...
  try {
    ...
    console.log(`Uploading file to Gemini File API: ${fileName} (${fileSize} bytes, ${mimeType})`);
    ...
    if (!initRes.ok) {
      ...
      console.warn(`Gemini upload init failed: ${initRes.status} - ${errText}`);
      return null;
    }

    const uploadUrl = initRes.headers.get('x-goog-upload-url');
    if (!uploadUrl) {
      console.warn('Upload URL not found in response headers');
      return null;
    }
    ...
    if (!uploadRes.ok) {
      ...
      console.warn(`Gemini upload finalize failed: ${uploadRes.status} - ${errText}`);
      return null;
    }

    const fileInfo = await uploadRes.json();
    console.log('Gemini File API Upload successful:', fileInfo?.file?.uri);

    return fileInfo?.file?.uri || null;
  } catch (error) {
    console.warn('Error uploading file to Gemini API:', error);
    return null;
  }
}
...
`;
  ...
  if (fileBuffer && fileName && mimeType) {
    fileUri = await uploadBufferToGemini(fileBuffer, fileName, mimeType);
  }
  ...
  console.warn('All Gemini AI endpoints failed. Using deterministic fallback parser.');
